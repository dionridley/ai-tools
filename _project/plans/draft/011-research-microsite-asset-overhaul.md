...
- [ ] The corpus index lists every research report (including a simulated legacy self-contained report added via repair) with title, date, one-line answer, and a working link — in both `index.md` and `index.html`, and the rendered view passed Dion's presentation review (helpful landing page, not a data dump).
...
- [ ] Corpus-index presentation review: iterate the index's rendered look with Dion on the fixture (visual hierarchy of title/date/answer, spacing, light index-specific styles if needed — may touch template `styles.css`; sync the fixture's `assets/v1` copy when it does) until it reads as a helpful landing page, not a list dump.
