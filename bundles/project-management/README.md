...
Initializes or updates project with the standard directory structure and a canonical AGENTS.md. As of v1.6.0, this is a **Skill 2.0** (`skills/dr-init/`) rather than a command — invocation is unchanged, internals are significantly upgraded.
...
- Generates **AGENTS.md**, the project's only agent-guidance file, carrying the versioned plugin sections. Every supported harness reads it (Claude Code natively since v2.1.277).
- **Moves an existing `CLAUDE.md` into AGENTS.md first**: Claude Code (v2.1.277+) reads AGENTS.md only when no CLAUDE.md exists, so /dr-init previews what will move, asks yes/no, then deletes CLAUDE.md. Answering no stops /dr-init without changes.
- **Scope** — only manages plugin-specific sections (plan workflow, available commands, task completion protocol). For codebase-specific documentation (architecture, build/test/lint commands, coding conventions), ask your coding agent to add it to AGENTS.md
- **Three detection states**:
  - *Fresh* — no `AGENTS.md`: scaffolds everything from scratch
  - *Already initialized* — plugin marker in AGENTS.md: verifies structure, compares section versions, offers a diff-preview of any outdated or missing sections (and any outdated plugin wording) before updating. Pre-3.0.0 layouts are also offered the `_claude/` → `_project/` directory rename
  - *Has AGENTS.md, no plugin structure* — shows a full preview of what will be appended and asks before writing
- **Diff preview** before any update to existing files
- **Git safety preflight** — warns if `AGENTS.md` has uncommitted changes before modifying it. The skill never runs state-modifying git commands except the user-approved `git mv _claude _project` migration; commits are your responsibility
...
- **Populates Definition of Done** from project config files (`AGENTS.md`, `package.json`, `Cargo.toml`, etc.) — the test/lint/typecheck commands every phase must satisfy.
...
### AGENTS.md was modified unexpectedly

**Problem**: AGENTS.md contents changed after running commands.

**Clarification**: `/dr-init` is the only part of this plugin that ever writes to `AGENTS.md`, and it always shows a preview and asks for confirmation before writing. It modifies it in four cases, all user-approved:
1. **Fresh scaffold** — creates AGENTS.md from the plugin template when none exists
2. **Section update** — rewrites plugin-managed sections in AGENTS.md when their version markers are outdated (diff shown first)
3. **Append** — adds plugin-managed sections to an existing AGENTS.md when no plugin structure exists yet (full preview shown first)
4. **Instruction-file move** — moves another instruction file's content to the end of AGENTS.md (see `/dr-init` above; always previewed and asked first)

If the file changed without you running `/dr-init`, check `git log -- AGENTS.md` — a different tool, editor, or user likely made the change.
